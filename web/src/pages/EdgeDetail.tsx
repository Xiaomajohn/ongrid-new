...
export default function EdgeDetailPage() {
  ...
  // Linked device shown in the header so users can jump to the host
  // detail page from an agent detail page. device_id is the only link
  // edge → device, so we resolve it on demand after the edge loads.
  const [hostDevice, setHostDevice] = useState<{ id: number; name: string } | null>(null);
  useEffect(() => {
    ...
    getDevice(edge.device_id)
      .then((d) => {
        if (cancelled) return;
        setHostDevice({ id: d.id, name: d.name });
      })
      .catch(() => {
        if (cancelled) return;
        setHostDevice(null);
            ...
            <div className="min-w-0">
              ...
              {hostDevice && (
                <div className="mt-1 flex items-center gap-1.5 truncate text-[11px] text-zinc-500">
                  <span>{tr('所属设备', 'Host device')}:</span>
                  <button
                    type="button"
                    onClick={() => navigate(`/hosts/${hostDevice.id}`)}
                    className="rounded px-1.5 py-0.5 font-medium text-zinc-300 hover:bg-zinc-800 hover:text-zinc-100"
                    title={tr('查看所属设备', 'View host device')}
                  >
                    {hostDevice.name} <span className="text-zinc-600">#{hostDevice.id}</span>
                  </button>
