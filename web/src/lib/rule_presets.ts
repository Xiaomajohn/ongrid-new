...
const PRESET_DEFS: RulePresetDef[] = [
  ...
  },
  {
    ...
    hintZh: 'Prom 直接 scrape AI 运维平台 manager 失败 ⇒ manager 进程挂了 / 网络断了。比 prom_write_total 可靠（不需要 remote_write 通畅）。',
    hintEn: "Prom scrape of the AI Ops Platform manager fails — manager process down or network broken. More reliable than prom_write_total (doesn't depend on remote_write).",
